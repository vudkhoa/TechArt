                                                    ...
                                                    previewRect))
            {
                ...
                using (new NewIndentLevel(0))
                {
                    using (new NewWideModeScope(position.width > 350))
                    {
                        ...
                        {
                            GUIContent label = new(PropertyConversions.NameFormat(prop.name), prop.tooltip);
                            Rect lineRect = new(x: spacing,
                                                y: height,
                                                width: previewRect.width - 2 * spacing,
                                                height: DrawProperties.GetPropertyHeight(label, prop));

                            EditorGUI.PropertyField(lineRect, prop, label, true); // we are using EditorGUI.PropertyField instead of DrawProperties.PropertyField, so it looks like in the default inspector
