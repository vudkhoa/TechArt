    ...
    public class SetDrawer : TypedPropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (draggedObject.Any())
            {
                ...
                {
                    property.CallMethodInside("TryAdd", new object[] { obj });
                }
                if (!property.ApplyModifiedField(true))
                {
                    Debug.LogWarning($"No Elements added on DragNDrop: Either no Objects of type '{argType}' were dragged or dragged objects were already present.");
                }
            }

            // draw header
            DrawProperties.ListDrawer.DrawHeader(headerRect, label, values);
